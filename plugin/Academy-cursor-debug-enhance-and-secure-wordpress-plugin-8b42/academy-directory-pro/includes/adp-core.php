...
class ADP_Core {
    public function init() {
        ...
        // ثبت متاهای اصلی و نمایش در REST API
        add_action( 'init', array( $this, 'register_core_meta' ) );
    ...
    public function register_core_meta() {
        $meta_fields = array(
            'phone'       => array( 'type' => 'string',  'sanitize_callback' => function( $val ) { return preg_replace( '/[^0-9+\-\s()]/', '', (string) $val ); } ),
            'website'     => array( 'type' => 'string',  'sanitize_callback' => 'esc_url_raw' ),
            'instagram'   => array( 'type' => 'string',  'sanitize_callback' => 'esc_url_raw' ),
            'subjects'    => array( 'type' => 'string',  'sanitize_callback' => 'sanitize_text_field' ),
            'rating'      => array( 'type' => 'number',  'sanitize_callback' => function( $val ) { $v = intval( $val ); return max( 1, min( 5, $v ) ); } ),
            'priority'    => array( 'type' => 'integer', 'sanitize_callback' => 'absint' ),
            'embed'       => array( 'type' => 'string',  'sanitize_callback' => function( $val ) {
                $allowed = array( 'iframe' => array( 'src' => true, 'width' => true, 'height' => true, 'style' => true, 'allowfullscreen' => true, 'loading' => true, 'referrerpolicy' => true, 'frameborder' => true ) );
                return wp_kses( (string) $val, $allowed );
            } ),
            'featured'    => array( 'type' => 'string',  'sanitize_callback' => function( $val ) { return in_array( $val, array( 'yes', 'no' ), true ) ? $val : 'no'; } ),
            'lat'         => array( 'type' => 'number',  'sanitize_callback' => function( $val ) { $v = floatval( $val ); return $v ?: 1.0; } ),
            'lng'         => array( 'type' => 'number',  'sanitize_callback' => function( $val ) { $v = floatval( $val ); return $v ?: 1.0; } ),
            'field_style' => array( 'type' => 'string',  'sanitize_callback' => function( $val ) { $v = wp_strip_all_tags( (string) $val ); $v = preg_replace( '/<script[^>]*>.*?<\\/script>/is', '', $v ); $v = preg_replace( '/javascript:/i', '', $v ); return $v; } ),
        );

        foreach ( $meta_fields as $key => $cfg ) {
            register_post_meta( 'academy', '_adp_' . $key, array(
                'type'              => $cfg['type'],
                'single'            => true,
                'show_in_rest'      => true,
                'sanitize_callback' => $cfg['sanitize_callback'],
    ...
    public function activate() {
        ADP_CPT::register();
        // ایجاد ترم‌های پیش‌فرض taxonomy نوع موجودیت
        ADP_CPT::ensure_default_terms();
        flush_rewrite_rules();
